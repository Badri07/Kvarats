...
@Component({
  selector: 'app-cheif-complaint',
  standalone: false,
  templateUrl: './cheif-complaint.component.html',
  styleUrl: './cheif-complaint.component.scss'
})
export class CheifComplaintComponent {
...
loadMedications(): void {
    this.dropdownService.getCheifComplaints(this.currentPage, this.paginationPageSize, this.searchText).subscribe((response: any) => {
...
onSubmit(): void {
  if (this.medicationForm.valid) {
    ...
    if (this.isEditing && this.editingId) {
      ...
      this.dropdownService.updatecheifComplaints(payload).subscribe({
        next: (res: any) => {
          if (res) {
      ...
      this.dropdownService.createCheifComplaints(formData).subscribe({
        next: (res: any) => {
          if (res) {
...
deleteMedication(): void {
  if (this.deleteId) {
    this.dropdownService.deleteCheficomplaints(this.deleteId).subscribe({
      next: () => {
        this.loadMedications();
      ...
      if (this.currentPage < this.totalPages) {
        ...
        uploadFile(): void {
          ...
          private uploadToBackend(file: File): void {
            this._loader.show();
            this._superadminService.UploadFileCheifComplaint(file).subscribe({
              next: (res) => {
                this.loadMedications();
          ...
          private sampleFile: string = 'https://careslot-dev.s3.amazonaws.com/ChiefComplaint/chief_complaints_sample.csv?AWSAccessKeyId=AKIAXJSU3GUHIVYCBLG7&Expires=2078390736&Signature=aZw64xtvbUKXQF%2FoMuxKbPU2SAU%3D';
