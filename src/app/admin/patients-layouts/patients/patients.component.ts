...
// });
  ...
  therapistList: any[] = [];
    ...
    this.adminservice.getPatientsList(clientId, this.patientCurrentPage, this.patientPageSize).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        ...
        this._toastr.error('Failed to load patients list');
      }
    });
  }
    loadTherapists(): void {
    this.adminservice.getTherapistList().subscribe({
      next: (res: any[]) => {
        this.therapistList = res || [];
      },
      error: (err) => {
        console.error('Failed to load therapists:', err);
        this._toastr.error('Failed to load therapists list');
      }
    });
  }
...
private async loadInitialAssessmentData(): Promise<void> {
  ...
  this.patientData = {
    patientids:this.patientData.patientids,
    therapistids:this.patientData.therapistids,
...
onAllergyFocus(): void {
  ...
  addNewItem(type: DropdownType): void {
    ...
    this.triggerAutoSave();
      ...
      const optionsArray = this[optionsField] as any[];
